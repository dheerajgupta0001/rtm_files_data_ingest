...
def insertDaywiseVoltMetrics(appDbConnStr: str, dataSamples: List[IVoltDataRecord]) -> bool:
    """Inserts a entity metrics time series data into the app db

    Args:
    appDbConnStr (str): [description]
    dataSamples (List[IVoltDataRecord]): [description]
    ...
    try:
        ...
        keyNames = ['data_time', 'volt_level',
                    'entity_name', 'metric_name', 'data_val']
        colsNames = ['data_time', 'volt_level',
                     'entity_name', 'metric_name', 'data_val']
        sqlPlaceHldrsTxt = ','.join([':{0}'.format(x+1)
                                     for x in range(len(colsNames))])
        # delete the rows which are already present
        existingVoltRecords = [(x['data_time'], x['volt_level'], x['entity_name'], x['metric_name'])
        ...
        print('Error while insertion of daily volt metrics')
        print(err)
    finally:
