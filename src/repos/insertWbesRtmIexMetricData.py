...
def insertWbesRtmIexData(appDbConnStr: str, dataSamples: List[IWbesRtmIexDataRecord]) -> bool:
    ...
    try:
        ...
        dbCur.executemany(
                    "delete from MO_WAREHOUSE.WBES_RTM_IEX where TIME_STAMP=:1\
                        and beneficiary=:2 and beneficiary_type=:3", existingEntityRecords)
        # insert the raw data
        sql_insert = "insert into MO_WAREHOUSE.WBES_RTM_IEX({0}) values ({1})".format(
        ...
        print('Error while insertion of WBES RTM IEX Data')
        print(err)
