...
def insertPxiRtmData(appDbConnStr: str, dataSamples: List[IPxiRtmDataRecord]) -> bool:
    ...
    try:
        ...
        dbCur.executemany(
            "delete from MO_WAREHOUSE.PXI_RTM where TIME_STAMP=:1 and col_attributes=:2", existingEntityRecords)
        # insert the raw data
        sql_insert = "insert into MO_WAREHOUSE.PXI_RTM({0}) values ({1})".format(
        ...
        print('Error while insertion of PXI RTM Data')
        print(err)
