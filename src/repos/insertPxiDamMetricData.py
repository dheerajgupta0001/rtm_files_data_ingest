...
def insertPxiDamData(appDbConnStr: str, dataSamples: List[IPxiDamDataRecord]) -> bool:
    ...
    try:
        ...
        dbCur.executemany(
            "delete from MO_WAREHOUSE.PXI_DAM where TIME_STAMP=:1 and col_attributes=:2", existingEntityRecords)
        # insert the raw data
        sql_insert = "insert into MO_WAREHOUSE.PXI_DAM({0}) values ({1})".format(
        ...
        print('Error while insertion of PXI Dam Data')
        print(err)
