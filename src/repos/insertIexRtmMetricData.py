...
def insertIexRtmData(appDbConnStr: str, dataSamples: List[IIexRtmDataRecord]) -> bool:
    ...
    try:
        ...
        dbCur.executemany(
            "delete from MO_WAREHOUSE.IEX_RTM where TIME_STAMP=:1 and col_attributes=:2", existingEntityRecords)
        # insert the raw data
        sql_insert = "insert into MO_WAREHOUSE.IEX_RTM({0}) values ({1})".format(
        ...
        print('Error while insertion of Iex Rtm Data')
        print(err)
