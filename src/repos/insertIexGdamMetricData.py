...
def insertIexGdamData(appDbConnStr: str, dataSamples: List[IIexGdamDataRecord]) -> bool:
    ...
    try:
        ...
        dbCur.executemany(
            "delete from MO_WAREHOUSE.IEX_GDAM where TIME_STAMP=:1 and col_attributes=:2", existingEntityRecords)
        # insert the raw data
        sql_insert = "insert into MO_WAREHOUSE.IEX_GDAM({0}) values ({1})".format(
        ...
        print('Error while insertion of Iex Gdam Data')
        print(err)
